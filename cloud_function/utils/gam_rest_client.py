...
class GAMRestClient:
    ...
    def convert_month_year(value: int) -> date:
        ...
        Note: At year boundaries, GAM may overflow the month (e.g., 1512
        instead of 1600 for January 2026). We handle this with modular
        arithmetic.
        ...
        Example:
            1510 → 2025-11-01 (year offset 15 + month 10 (0-indexed) = November)
            1511 → 2025-12-01 (year offset 15 + month 11 (0-indexed) = December)
            1512 → 2026-01-01 (year offset 15 + month 12 overflows to next year)
            1600 → 2026-01-01 (year offset 16 + month 0 = January)
        """
        year_offset = value // 100
        month_0indexed = value % 100

        # Handle month overflow at year boundaries (e.g., month=12 instead of rolling to next year)
        year = 2010 + year_offset + (month_0indexed // 12)
        month = (month_0indexed % 12) + 1
    ...
    def get_fill_rate_report(self) -> pd.DataFrame:
        ...
        # Aggregate by (date, ad_unit_name) to deduplicate rows from different
        # hierarchy levels (e.g., hp appearing at multiple AD_UNIT_NAME_ALL_LEVEL)
        sum_cols = ["unfilled_impressions", "code_served_count", "responses_served",
                    "ad_server_impressions", "ad_requests"]
        agg_dict = {col: "sum" for col in sum_cols if col in df.columns}
        df = df.groupby(["date", "ad_unit_name"], as_index=False).agg(agg_dict)
        # Recalculate fill_rate from aggregated values
        df["fill_rate"] = df["ad_server_impressions"] / df["ad_requests"].replace(0, pd.NA)
